...
export default function PostPage({ params }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const [r1, r2] = await Promise.all([fetch(`/api/posts/${id}`), fetch("/api/tags")]);
  ...
  if (!post) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        <h2 className="text-xl font-semibold text-red-800 mb-2">Post no encontrado</h2>
        <p className="text-red-600 mb-4">El post que buscas no existe.</p>
        <Link href="/posts" className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition-colors">
          Volver a Posts
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Editar Título */}
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">✏️ Editar Título</h2>
      ...
      {/* Tags del Post */}
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">🏷️ Tags del Post</h2>
        
        {/* Tags actuales */}
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-700 mb-3">Tags Asignados</h3>
        ...
        <div className="border-t pt-4">
          <h3 className="text-lg font-medium text-gray-700 mb-3">Agregar Tag</h3>
          <form onSubmit={addTag} className="flex space-x-3">
            ...
            >
              ...
              {tags.filter(t => !post.postTags?.some(pt => pt.tagId === t.id)).map((t) => (
